    ...
    private void handleUpdateInfor(){
        btnUpdateInfor.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(isAdmin == 1){
                    transaction.replace(R.id.admin_menu_container, updateInforFragment);
                } else {
                    transaction.replace(R.id.container, updateInforFragment);
                }
                transaction.addToBackStack(null); // Thêm fragment vào ngăn xếp
                transaction.commit();
            }
        });
    }
